...
import { generateTenantReportDocx } from "@/lib/hse-file-generators";
...
export async function GET(request: Request, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  const report = await generateTenantReportDocx(tenantId);
  ...
  return new Response(new Uint8Array(report), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="rapport_entreprise_${tenantId}.docx"`,
    },
  });
}
