...
import { generateModuleReportPdf } from "@/lib/hse-file-generators";
...
export async function GET(request: Request, { params }: { params: Promise<{ moduleId: string }> }) {
  ...
  const report = await generateModuleReportPdf(moduleId);
  ...
  return new Response(new Uint8Array(report), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="rapport_module_${moduleId}.pdf"`,
    },
  });
}
